...
st.set_page_config(page_title="Observatoire des Imaginaires", page_icon="herb", layout="wide")
...
### A. Sidebar
with st.sidebar:
	st.image('https://media.licdn.com/dms/image/D4E0BAQEZHVBxFn3OXQ/company-logo_200_200/0/1697116934909/cercle_thmatique_culture_the_shifters_logo?e=1718841600&v=beta&t=_2DWaEBrblIgXhgVASUipHTcJesOL6s1Sk2uH73Kx58', use_column_width=True)# width=50 
...
#cont_metric.header('Aperçu des données')
...
df_truncated.insert(3,"pays_rework",[pays if len(pays.split(';')) == 1 else 'INTERNATIONAL' for pays in df_truncated['PAYS']])
...
with cont_metric:
	...
	with col_nb_editeur :
		#Metric nb Editeurs
		st.metric(label="Films", value = len(set(df_truncated[df_truncated.TYPE == "FILM"]['TITRE'])))
	with col_nb_prem_roman : 
		#Metric Premier Roman
		st.metric(label="Séries", value = len(set(df_truncated[df_truncated.TYPE == "SÉRIE"]['TITRE'])))
		#st.divider()

	st.write()
	st.write(f":blue[{round(100*len(set(df_truncated[df_truncated.TYPE == 'FILM']['TITRE']))/len(set(df_truncated['TITRE'])),2)}%] des contenus renseignés sont des films vs :blue[{round(100*len(set(df_truncated[df_truncated.TYPE == 'SÉRIE']['TITRE']))/len(set(df_truncated['TITRE'])),2)}%] des séries.")
...
#titles_more_than_once = titles_more_than_once[titles_more_than_once > 1].reset_index().rename(columns={"index": "TITRE", "TITRE": "Count"})


titles_more_than_once = df_truncated.groupby(['TITRE','TYPE']).agg(compte=("TITRE","count")).reset_index()
titles_more_than_once = titles_more_than_once[titles_more_than_once['compte'] > 1]

#st.dataframe(titles_more_than_once)
...
with st.container(border=True):
	...
	col_freq_film_select, col_freq_film_vide, col_freq_film_graph = st.columns([2,0.5,5])
	with col_freq_film_select :
		type_choice = st.selectbox("Choisir un type", titles_more_than_once['TYPE'].unique(), index=None)
	with col_freq_film_graph :
		if type_choice == 'FILM':
			t = titles_more_than_once.loc[titles_more_than_once['TYPE'] == 'FILM'].sort_values(by='compte', ascending=True)
		elif type_choice == 'SÉRIE':
			t = titles_more_than_once.loc[titles_more_than_once['TYPE'] == 'SÉRIE'].sort_values(by='compte', ascending=True)
		else :
...
with st.container(border=True):
	...
	with col_contenu_date :
		date_group_df = df_truncated.groupby('ANNEE').count().reset_index()[['ANNEE', "TITRE"]].rename(columns={"TITRE":"nb_titre"})
		date_group_df['periode_percent'] = 100*(1-(date_group_df.nb_titre.cumsum() / date_group_df.nb_titre.sum()))
		
		date_min = str(df_truncated.ANNEE.min())
		date_max = str(df_truncated.ANNEE.max())
		date_pareto = date_group_df[date_group_df['periode_percent']<=80]['ANNEE'].min()
		date_value_pareto = int(round(date_group_df[date_group_df['periode_percent']<=80]['periode_percent'].max(),0))
	
		st.markdown(f"Les contenus datent d’une période qui s’étend de {date_min} à {date_max}. {date_value_pareto}% des contenus sont postérieurs à {date_pareto}.")

		st.bar_chart(date_group_df, x='ANNEE', y="nb_titre")
		
	with col_contenu_graph:
		country_group_df = df_truncated
		country_group_df = country_group_df.groupby('pays_rework').count().reset_index()[['pays_rework',
							"TITRE"]].rename(columns={"TITRE":"nb_titre"}).sort_values('nb_titre')
		country_group_df['country_percent_cumul'] = round(100*(1-(country_group_df.nb_titre.cumsum() / country_group_df.nb_titre.sum())),0)
		country_group_df['country_percent'] = round(100*(country_group_df.nb_titre / country_group_df.nb_titre.sum()),2)

		country_value_pareto = int(round(country_group_df[country_group_df['country_percent']>=10]['country_percent'].sum(),2))
		country_group_df_pareto = country_group_df[country_group_df['country_percent']>=10][['pays_rework','country_percent']].sort_values('country_percent', ascending = False)

		st.write(f"A **:blue[{country_value_pareto}%]**, les 2 principaux pays dont les contenus sont les plus visionnés sont : {country_group_df.nlargest(2,'country_percent').reset_index(drop=True)['pays_rework'][0].capitalize()} ({country_group_df.nlargest(2,'country_percent').reset_index(drop=True)['country_percent'][0]}%) et {country_group_df.nlargest(2,'country_percent').reset_index(drop=True)['pays_rework'][1].capitalize()} ({country_group_df.nlargest(2,'country_percent').reset_index(drop=True)['country_percent'][1]}%).")

		fig_type=px.bar(country_group_df, y='pays_rework', x='nb_titre', orientation='h', text_auto=True)
		st.plotly_chart(fig_type, use_container_width=True)

#LIEUX VISIONNAGE
with st.container (border=True):
	canal_group_df = df_truncated.groupby('CANAL').count()[["TITRE"]].rename(columns={"TITRE":"nb_titre"}).sort_values('nb_titre', ascending = False)
	canal_group_df['canal_percent'] = 100*(canal_group_df.nb_titre / canal_group_df.nb_titre.sum())
	canal_country_group_df = df_truncated.groupby(['CANAL','pays_rework']).count()[["TITRE"]].rename(columns={"TITRE":"nb_titre"}).sort_values('nb_titre', ascending = False).reset_index()
...
st.dataframe(df_truncated[["TITRE","TRAITEMENT DU RECIT","TYPE DE MONDE"]].groupby(["TRAITEMENT DU RECIT","TYPE DE MONDE"]).count())
